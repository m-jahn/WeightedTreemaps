...
The C++ code computing the actual Voronoi tesselation requires the [CGAL](https://www.cgal.org/download.html) library headers. Thanks to [Ahmadou Dicko](https://github.com/dickoa), installing the complete CGAL library locally is no longer necessary. Instead, the package depends on the CGAL4 headers that are supplied as R package `cgal4h` on Ahmadou's gitlab page. In case automatic retrieval of the package does not work during `SysbioTreemaps` installation, install `cgal4h` manually before.
